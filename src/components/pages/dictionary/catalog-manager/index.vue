...
export default class CatalogModule extends Vue {
  ...
  private async beforeEditDictionary(id: number) {
    ...
    > = await HttpRequest.DictionaryModule.getCatalogInfoById({ id });
  ...
  private handleDicDelete(id: number) {
    ...
    Modal.confirm({
      ...
      async onOk() {
        const res: ApiResponse<
          boolean
        > = await HttpRequest.DictionaryModule.getCatalogDelete({ id });
  ...
  private async queryDictionaryList() {
    ...
    > = await HttpRequest.DictionaryModule.getCatalogList(this.searchParams);
